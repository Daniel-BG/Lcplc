...
public class Compressor {
	...
	private int maxBlockLines = 16;
	private int maxBlockSamples = 16;
	...
	public void setBlockSize(int lines, int samples) {
		this.maxBlockLines = lines;
		this.maxBlockSamples = samples;
	}
	
	public void setSQDownscale(int SQDownscale) {
		this.downscale = SQDownscale;
	}
	...
	public void compress(HyperspectralImageData hid, BitOutputStream outputStream) throws IOException {
		...
		for (int l = 0; l < imgLines; l += maxBlockLines) {
			for (int s = 0; s < imgSamples; s += maxBlockSamples) {
				boolean lastBlock = s >= imgSamples - 1 && l >= imgLines - 1;
				
				int blockBands = imgBands;
				int blockLines = Math.min(maxBlockLines, imgLines - l);
				int blockSamples = Math.min(maxBlockSamples, imgSamples - s);
				...
				//compress block
				try {
					this.compressBlock(block, blockBands, blockLines, blockSamples, lastBlock,  bos);
				} catch (IOException e) {
					e.printStackTrace();
					System.exit(0);
				}
	...
	private void compressBlock(int[][][] block, int bands, int lines, int samples, boolean flushBlock, BitOutputStream bos) throws IOException {
		...
		double delta = (double) (1 << this.downscale);
	...
	public HyperspectralImageData uncompress(ImageDataType idt, int bands, int lines, int samples, BitInputStream bis) {
		HyperspectralImageData hid = new HyperspectralImageIntegerData(idt, bands, lines, samples);
		
		long time = System.nanoTime();
		///////
		
		for (int l = 0; l < lines; l += maxBlockLines) {
			for (int s = 0; s < samples; s += maxBlockSamples) {
				int blockBands = bands;
				int blockLines = Math.min(maxBlockLines, lines - l);
				int blockSamples = Math.min(maxBlockSamples, samples - s);
				
				//uncompress block
				try {
					int[][][] partialResult = this.uncompressBlock(blockBands, blockLines, blockSamples, bis);
					for (int i = 0; i < blockBands; i++) {
						for (int j = 0; j < blockLines; j++) {
							for (int k = 0; k < blockSamples; k++) {
								hid.setDataAt(partialResult[i][j][k], i, j+l, k+s);
							}
						}
					}
				} catch (IOException e) {
	...
	public int[][][] uncompressBlock(int bands, int lines, int samples, BitInputStream bis) throws IOException {
		int[][][] decodedBlock = new int[bands][lines][samples];
 ...
 * 
	...
	public void test(HyperspectralImageData hid) {		
		int imgBands	= hid.getNumberOfBands();
		int imgLines	= hid.getNumberOfLines();
		int imgSamples	= hid.getNumberOfSamples();
		this.SAMPLE_DEPTH = hid.getDataType().getBitDepth();
		...
		for (int l = 0; l < imgLines; l += MAX_LINES_PER_BLOCK) {
			for (int s = 0; s < imgSamples; s += MAX_SAMPLES_PER_BLOCK) {
				...
				//compress block
				try {
					this.compressBlock(block, blockBands, blockLines, blockSamples, compressedBlocks == BLOCKS_TO_CODE - 1,  bos);
				} catch (IOException e) {
					e.printStackTrace();
					System.exit(0);
				}
